...
const InstallAppPrompt = () => {
  ...
  useEffect(() => {
    ...
    if (isMobile) {
      ...
      if (!dismissed) {
        // ONLY force show for iOS, because Android relies on beforeinstallprompt
        if (isIOSDevice) {
  ...
  return (
    <div style={{
      ...
      {isIOS ? (
        ...
        <div style={{ background: '#F2F2F7', padding: '12px', borderRadius: '12px', fontSize: '14px', color: '#333' }}>
          To install, tap the <b>three dots menu</b> in your browser (top right) and select <b>Install app</b>.
        </div>
      )}
    </div>
  );
